...
#if 0
static bool is_block_task(struct task_struct *p)
{
	if (strstr(current->group_leader->comm, "SDLActivity")) {
			pr_err("rvh_sched_setaffinity: THREAD: %s (pid=%d) | PROCESS: %s (tgid=%d).\n",
            current->comm, current->pid, current->group_leader->comm, current->tgid);
		return true;
	}
